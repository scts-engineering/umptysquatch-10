...
#define USE_REAL_SERVOS
void setServo(int num, float degrees) {

#ifdef USE_REAL_SERVOS
  float microseconds = degrees * 9.9 + 870;
  switch(num) {
    case 1:
      servoa.writeMicroseconds(microseconds);
      break;
    case 2:
      servob.writeMicroseconds(microseconds);
      break;
    case 3:
      servoc.writeMicroseconds(microseconds);
      break;
    case 4:
      servod.writeMicroseconds(microseconds);
      break;
  }
#endif
#ifdef USE_TEST_SERVOS
  switch(num) {
    case 1:
      servoa.write(degrees);
      break;
    case 2:
      servob.write(degrees);
      break;
    case 3:
      servoc.write(degrees);
      break;
    case 4:
      servod.write(degrees);
      break;
  }
#endif
}
...
void loop() { // start of the main loop of the program
 ...
 /* 
  ...
  float y = (analogRead(A3) - 512) / 512.0f;

  float servo1angle = (atan(x - y) * -180 / PI) + 90;
  float servo2angle = (atan(x + y) * -180 / PI) + 90;
  float servo3angle = (atan(y - x) * -180 / PI) + 90;
  float servo4angle = (atan(x + y) * 180 / PI) + 90;
  ...
  if (servo4angle > 135) {
    servo4angle = 135;
  } else if (servo4angle < 45) {
    servo4angle = 45;
  }

  int deadmin = 85;
  int deadmax = 95;
  if(servo1angle > deadmin && servo1angle < deadmax) servo1angle = 90;
  if(servo2angle > deadmin && servo2angle < deadmax) servo2angle = 90;
  if(servo3angle > deadmin && servo3angle < deadmax) servo3angle = 90;
  if(servo4angle > deadmin && servo4angle < deadmax) servo4angle = 90;
  
  servo1angle = round(servo1angle);
  servo2angle = round(servo2angle);
  servo3angle = round(servo3angle);
  servo4angle = round(servo4angle);

  setServo(1, servo1angle);
  setServo(2, servo2angle);
  setServo(3, servo3angle);
  setServo(4, servo4angle);
